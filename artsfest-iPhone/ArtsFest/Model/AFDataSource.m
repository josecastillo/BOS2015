...
- (void)loadNextFeed {
	...
	NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"http://127.0.0.1:5000/%@.json?updated_since=%.0f", endpoint, [lastUpdated doubleValue]]];
	...
	if (response) {
		...
		NSUInteger numResults = [result count];
		int processed = 0;
